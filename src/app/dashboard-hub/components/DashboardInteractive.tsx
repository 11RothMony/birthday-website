...
const DashboardInteractive = () => {
  ...
  useEffect(() => {
    ...
    const birthdaysData: Birthday[] = mockData.staff
      ...
      .map(staff => ({
        ...
        alt: staff.alt,
        age: staff.age,
        cakeStatus: (staff.cakeStatus as 'ordered' | 'ready' | 'delivered' | 'not-ordered') || 'not-ordered',
      }))
      .sort((a, b) => {
        const dateA = new Date(mockData.staff.find(s => parseInt(s.id) === a.id)!.birthday).getDate();
        const dateB = new Date(mockData.staff.find(s => parseInt(s.id) === b.id)!.birthday).getDate();
        return dateA - dateB;
      });
    ...
    const eventsData: TimelineEvent[] = birthdaysData.map((birthday, index) => {
      const statusMap: Record<string, { icon: string; color: 'primary' | 'trust' | 'celebration' | 'success'; action: string }> = {
        'delivered': { 
  ...
  const handleUpdateStatus = (id: number, status: 'ordered' | 'ready' | 'delivered' | 'not-ordered') => {
